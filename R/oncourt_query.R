...
#' Oncourt database connection
#'
#' Before trying connect to OnCourt database, to simplify the process, please set-up environment variables by using usethis::edit_r_environ()
#' .Two variables are needed :
#' * *oncourt_path* - path to Oncourt database file, e.g. on Windows: C:/Program Files (x86)/OnCourt/OnCourt.mdb
#' * *oncourt_pwd*  - password to database
#' @md
#' @param db_path path to Oncourt Access database file
#' @param pw password (contact with OnCourt administrators)
#' @return connector to database
#' @examples
# cn_access = connect_oncourt_db( )
connect_oncourt_db = function(db_path = Sys.getenv('oncourt_path'), pw = Sys.getenv('oncourt_pwd')){
  RODBC::odbcConnectAccess2007(access.file = db_path, pwd = pw)
}
...
#'
#' Get actual and historical ranking informations
#' @md
#' @param type wta or atp
#' @param from lower bound of ranking release date expressed in integer number, e.g. 730, from 2 years back to now
#' @param con  connector to database
#' @return data frame with ranking data
#' * *DATE_R*  - date release of ranking
#' * *ID_P_R*  - ID of player
#' * *POINT_R* - points
#' * *POS_R*   - position
#' * *DATE_P*  - date of birth
#' * etc...
#' @examples
# df_rank = oncourt::query_oncourt_playerranking(type = 'wta', from = 730)
query_oncourt_playerranking = function(type, from = 730, con = cn_access){

  # if(!is.integer(from))
  RODBC::sqlQuery(con, paste("SELECT distinct *
                                           FROM ratings_type INNER JOIN players_type ON ratings_type.ID_P_R = players_type.ID_P
                                           WHERE (((ratings_type.DATE_R)>Now()-", from, "))
...
#' Get schedule
#'
#' Get actual schedule of single & double matches
#' @md
#' @param type wta or atp
#' @param con  connector to database
...
#' * etc...
#' @examples
# df_schedule = oncourt::query_oncourt_schedule(type = 'wta')
...
#'
#' Get historical matches with statistics
#' @md
#' @param type wta or atp
#' @param date_min lower bound of date
#' @param date_max upper bound of date, default today's date
#' @param players_id ID's of players to querying, e.g. column ID_P_R from query_oncourt_playerranking()
#' @param con  connector to database
#' @return data frame with historical data of particual players
#' * *type*     - atp or wta
#' * *score*    - win or lost from perspective of player 1 (P1)
#' * *ACES_1*   - aces of player 1
#' * *DF_1*     - double faults of player 1
#' * *TPW_1*    - total points won
#' * *TPW*      - TPW_1 + TPW_2
#' * *TIME_G*   - length of match, in minutes
#' * etc...
#' @examples
# df_player = oncourt::query_oncourt_playerdata(type = 'wta', players_id = c(45854))
query_oncourt_playerdata = function(type, date_min = as.Date(Sys.time()) - 365*4, date_max = as.Date(Sys.time()), players_id = c(45854, 54663 ), con = cn_access){

  players_id = players_id %>% unique() %>% t() %>% paste( collapse = ',')
...
#' Get historical data
#'
#' Get historical matches constrained by date
#' @md
#' @param type wta or atp
#' @param date_min lower bound of date
#' @param date_max upper bound of date, default today's date
